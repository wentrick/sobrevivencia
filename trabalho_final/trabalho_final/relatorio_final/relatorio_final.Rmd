...
# packages
pacman::p_load(readr,tidyverse,survival,AdequacyModel,rms,knitr,xtable,kableExtra,ggplot2,ggfortify,ranger,BART,randomForestSRC,qpcR,lmtest)
```
...
A unica variavel que nao foi significante e escolhemos o modelo sem ela foi `high_blood_pressure`. Em seguida deveriamos ajustar um modelo com as variaveis que restaram e compara com o modelo com as variaveis excluidas para ver se a retirada conjunta faz alguma diiferenca. Como so tivemos uma variavel removida nao precisamos realizar isso ja que repetiriamos o teste feito. 

\newpage

#### Etapa 3: Ajustar o modelo sem a variavel excluida e comparar com as variaveis removidas na etapa 1

O Objetivo dessa etapa é verfificar se alguma das variaveis excluidas na etapa 1 (`anaemia`,`creatinine_phosphokinase`,`diabetes`,`platelets`,`sex`,`smoking`) sao significativa em conjunto com outras selecionadas na etapa 2.
...
Nenhuma das variaveis que retiramos inicialmente ficam no modelo a nivel de 5%.

\newpage

#### Etapa 4: Verificar se podemos simplificar nosso modelo obtido na etapa 3 removendo alguma variavel.
...
Pelos resultados a nivel de 5% removemos a variavel `serum_sodium`. Com isso chegamos ao nosso modelo final para os efeitos principais

\newpage

#### Etapa 5: Verificar a interacao entre as variaveis restantes no modelo final obtido na etapa anterior.
...
Nenhum dos termos de interacao foram significativos para o modelo.
...
Temos 2 modelos finais e para isso vamos utilizar a analise de residuos de cada um para ver qual se adequa melhor aos dados
...
cat("Valores modelo manual usando funcoes:",manual)
```
...
```{r}
cat("Valores modelo stepwise manual:",medidalls)
cat("Valores modelo stepwise usando funcoes:",manual)
```


```{r}
AIC(lognormal.reduzida_step)
AIC(lognormal.reduzida)
```
```{r}
AICc(lognormal.reduzida_step)
AICc(lognormal.reduzida)
```


```{r}
BIC(lognormal.reduzida_step)
BIC(lognormal.reduzida)
```
