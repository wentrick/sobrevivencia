...
dados <- read_csv("heart_failure_clinical_records_dataset.csv") %>%
  dplyr::mutate(censura = DEATH_EVENT,
         tempo = time,
         age = round(age,0)) %>% 
...
Vamos observar como é a distribuicao das variaveis numericas do banco
...
Podemos notar que nosso estimador nao chega a 0, indicando a existencia de uma fracao de cura ou seja algumas observacoes nunca vao registrar o evento de interesse. No caso vamos estar comparando um modelo sem fracao de cura com um que leva isso em consideracao 
...
Aqui vemos que a variavel influencia na sobrevivnecia tendo em vista as curvas mais separadas e sem cruzzamentos, ou seja essa é uma variavel que podemos estar analisando no modelo final.
...
Podemos notar pela curva que nao existe diferenca significativa.
...
### Modelo Manual com fracao de cura
...
cat("Paramaetro de Escala do Modelo (Sigma):",results_expo$par[1])
cat("Taxa de cura do Modelo (Phi):",results_expo$par[2])
cat("Coeficientes do modelo:")
cat("Intercepto",results_expo$par[3])
cat("age:",results_expo$par[4])
cat("ejection_fraction",results_expo$par[5])
cat("serum_creatinine",results_expo$par[6])
```
...
### Modelo Stepwise com fracao de cura 
...
results_expo2 = optim(c(1.63,0.5,-0.336261,0.0403,-0.5081,0.0732,-0.046719),lognorm_vero,NULL,hessian = T,method = "BFGS")
```


```{r}

cat("Paramaetro de Escala do Modelo (Sigma):",results_expo2$par[1])
cat("Taxa de cura do Modelo (Phi):",results_expo2$par[2])
cat("Coeficientes do modelo:")
cat("Intercepto",results_expo2$par[3])
cat("age:",results_expo2$par[4])
cat("ejection_fraction",results_expo2$par[5])
cat("high_blood_pressure",results_expo2$par[6])
cat("serum_creatinine",results_expo2$par[7])
```


```{r}
logLE = (-1)*results_expo2$value
invRE = solve(results_expo2$hessian)
varianciaE = diag(invRE)
eppE = sqrt(varianciaE)

phi = results_expo2$par[2]
```
...
### residuos de cox snell
y = log(dados$tempo)
mip = cbind(x0,x1,x2,x3,x4) %*% results_expo2$par[3:length(results_expo2$par)] 
Smod = phi+(1-phi)*(1-pnorm((y-mip)/results_expo$par[1]))
ei = (-log(Smod))
